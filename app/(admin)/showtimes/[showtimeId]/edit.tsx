...
export default function EditShowtime() {
  ...
  const del = async () => {
    Alert.alert("Delete", "Are you sure you want to delete this showtime?", [
  ...
  return (
    <ScrollView contentContainerStyle={{ padding:16, gap:12 }}>
      <Text style={{ fontSize:20, fontWeight:"600" }}>Edit Showtime</Text>

      <Text>Movie ID</Text>
      <TextInput
        value={movieId}
        onChangeText={setMovieId}
        placeholder="Paste/enter Movie ID"
        style={{ borderWidth:1, borderColor:"#ccc", borderRadius:8, padding:10 }}
      />
      <Text style={{ color:"#666" }}>
        Tip: From Admin · Movies list, tap a movie to copy its ID (or wire a picker later).
      </Text>
